...
// Black Ops 1 SP
std::array<DBGameInfo, 1> GameBlackOps::SinglePlayerOffsets =
{{
    { 0xB741B8, 0xB73EF8, 0x3067C80, 0 }
}};
// Black Ops 1 MP
std::array<DBGameInfo, 1> GameBlackOps::MultiPlayerOffsets =
{{
    { 0xBF2C30, 0xBF2970, 0x3B6A880, 0 }
}};

// -- Finished with databases

bool GameBlackOps::LoadOffsets()
{
    // ----------------------------------------------------
    //    Black Ops 1 pools, DBAssetPools is an array of uint32 (ptrs) of each asset pool in the game
    //    The index of the assets we use are as follows: xanim (4), xmodel (5), xfx (0x1C)
    ...
    //    On Black Ops 1, "void" will be the first xmodel
    //    Black Ops 1 stringtable, check entries, results may vary
    //    Reading is (StringIndex * 16) + StringTablePtr + 4
    // ----------------------------------------------------
    ...
    if (CoDAssets::GameInstance != nullptr)
    {
        ...
        auto StringTableScan = CoDAssets::GameInstance->Scan("0F B7 C0 03 C6 83 C4 0C C1 E0 04");

        // Check that we had hits
        if (DBAssetsScan > 0 && StringTableScan > 0)
        {
            // Load info and verify
            auto GameOffsets = DBGameInfo(CoDAssets::GameInstance->Read<uint32_t>(DBAssetsScan - 0xB), CoDAssets::GameInstance->Read<uint32_t>(DBAssetsScan + 0x26), CoDAssets::GameInstance->Read<uint32_t>(StringTableScan + 0xC), 0);
            // Read required offsets (XANIM, XMODEL, XFX)
            CoDAssets::GameOffsetInfos.emplace_back(CoDAssets::GameInstance->Read<uint32_t>(GameOffsets.DBAssetPools + (4 * 4)));
            CoDAssets::GameOffsetInfos.emplace_back(CoDAssets::GameInstance->Read<uint32_t>(GameOffsets.DBAssetPools + (4 * 5)));
            CoDAssets::GameOffsetInfos.emplace_back(CoDAssets::GameInstance->Read<uint32_t>(GameOffsets.DBAssetPools + (4 * 0x1C)));
...
bool GameBlackOps::LoadAssets()
{
    ...
    if (NeedsAnims)
    {
        ...
        auto MaximumPoolOffset = (AnimationCount * sizeof(BOXAnim)) + AnimationOffset;
        // Store original offset
        auto MinimumPoolOffset = CoDAssets::GameOffsetInfos[0];

        // Store the placeholder anim
        BOXAnim PlaceholderAnim;
        // Clear it out
        ...
        {
            // Read
            auto AnimResult = CoDAssets::GameInstance->Read<BOXAnim>(AnimationOffset);
            ...
            {
                // Advance
                AnimationOffset += sizeof(BOXAnim);
                // Skip this asset
                continue;
            }
    ...
    if (NeedsModels)
    {
        ...
        // Calculate maximum pool size
        auto MaximumPoolOffset = (ModelCount * sizeof(BOXModel)) + ModelOffset;
        // Store original offset
        auto MinimumPoolOffset = CoDAssets::GameOffsetInfos[1];

        // Store the placeholder model
        BOXModel PlaceholderModel;
        // Clear it out
        ...
        {
            // Read
            auto ModelResult = CoDAssets::GameInstance->Read<BOXModel>(ModelOffset);
...
std::unique_ptr<XAnim_t> GameBlackOps::ReadXAnim(const CoDAnim_t* Animation)
{
    ...
    if (CoDAssets::GameInstance->IsRunning())
    {
        ...
        auto AnimData = CoDAssets::GameInstance->Read<BOXAnim>(Animation->AssetPointer);
        ...
        if ((_strnicmp(Animation->AssetName.c_str(), "viewmodel_", 10) == 0) || (_strnicmp(Animation->AssetName.c_str(), "t5_viewmodel_", 13) == 0))
        {
        ...
        // Read the delta data
        auto AnimDeltaData = CoDAssets::GameInstance->Read<BOXAnimDeltaParts>(AnimData.DeltaPartsPtr);
...
std::unique_ptr<XModel_t> GameBlackOps::ReadXModel(const CoDModel_t* Model)
{
    // Verify that the program is running
    if (CoDAssets::GameInstance->IsRunning())
    {
        // Read the XModel structure
        auto ModelData = CoDAssets::GameInstance->Read<BOXModel>(Model->AssetPointer);
        ...
        for (uint32_t i = 0; i < ModelData.NumLods; i++)
        {
            ...
            auto XSurfacePtr = ModelData.SurfacesPtr + (ModelData.ModelLods[i].SurfacesIndex * sizeof(BOXModelSurface));
            ...
            for (uint32_t s = 0; s < ModelData.ModelLods[i].NumSurfs; s++)
            {
                ...
                auto SurfaceInfo = CoDAssets::GameInstance->Read<BOXModelSurface>(XSurfacePtr);
                ...
                // Advance
                XSurfacePtr += sizeof(BOXModelSurface);
...
const XMaterial_t GameBlackOps::ReadXMaterial(uint64_t MaterialPointer)
{
    // Prepare to parse the material
    auto MaterialData = CoDAssets::GameInstance->Read<BOXMaterial>(MaterialPointer);
    ...
    {
        // Read the image info
        auto ImageInfo = CoDAssets::GameInstance->Read<BOXMaterialImage>(MaterialData.ImageTablePtr);
        // Read the image name (End of image - 8)
        auto ImageName = CoDAssets::GameInstance->ReadNullTerminatedString(CoDAssets::GameInstance->Read<uint32_t>(ImageInfo.ImagePtr + (sizeof(BOGfxImage) - 8)));
...
std::unique_ptr<XImageDDS> GameBlackOps::LoadXImage(const XImage_t& Image)
{
