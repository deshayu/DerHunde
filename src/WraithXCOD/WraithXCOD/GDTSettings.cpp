...
{
    // Ask to clear cache
    if (MessageBoxA(this->GetSafeHwnd(), "Are you sure you want to clear the GDT cache?", "Kobra", MB_OKCANCEL | MB_ICONQUESTION) == IDOK)
    {
        ...
        // Alert the user
        MessageBoxA(this->GetSafeHwnd(), "The GDT cache has been cleared", "Kobra", MB_OK | MB_ICONINFORMATION);
    }
}
